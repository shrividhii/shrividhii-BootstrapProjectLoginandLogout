...
namespace Connectivity.DAL.Helper
{
    public sealed class PostgreSQLHelper
    {
        ...
        public static string GetConnectString(string connectionname = "ConnectionPg")
        {
            string connectiostr = "ConnectionPg";
            try
               
            {
                if (connectionname == "ConnectionPg")
                    connectiostr = System.Configuration.ConfigurationManager.AppSettings["ConnectionPg"].ToString(); //"Server=3.12.97.34;Port=5432;Database=QNRU74;User Id=botdba;Password=6hwY!gef>8;CommandTimeout=10000;Pooling=false";
                else
                {
                    connectiostr = ConfigurationManager.AppSettings["ConnectionPg"].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataSet GetProceduredata(string ConnectString, string inputjson, string procedurename, out string result, out string Error, string outjson = "'outjson'")
        {
            ...
            NpgsqlParameter[] sqlParameters = new NpgsqlParameter[outjson_count];
            ...
            for (int i = 0; i < ds.Tables.Count; i++)
            {
                if (i == 0)
                {
                    ds.Tables[i].TableName = "Vidhi";
                }
                else
                {
                    ds.Tables[i].TableName = "Vidhi" + i;
                }
